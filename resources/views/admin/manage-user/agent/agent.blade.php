            ...
            <div class="form-wrap">
                ...
                <form action="" method="POST" id="agent_form">
                    <div class="row">
                        ...
                        @php $vendor_type = (empty($user)) ? '' : $user->vendor_type ; @endphp
                        <div class="col-lg-6">
                            <fieldset class="select-box">
                                <label for="vendor_type">Vendor type <strong class="text-danger">*</strong></label>
                                <select class="form-control" name="vendor_type" id="vendor_type" required>
                                    <option value="">Select type</option>
                                    <option value="1" @selected($vendor_type == '1') >Regular</option>
                                    <option value="2" @selected($vendor_type == '2') >Subscription</option>
                                </select>
                            </fieldset>
                        </div>
                        ...
                        @php
                            //$statehead_id = (empty($user)) ? '' : $user->agent_prfile->statehead_id;
                        @endphp
                        {{-- <div class="col-lg-6">
                            <fieldset class="select-box">
                                <label for="statehead_id">State Heads (Optional)</label>
                                <select class="form-control" name="statehead_id" id="statehead_id">
                                    <option value="">State Heads</option>
                                    @foreach ($allstateheads as $var)
                                    <option value="{{ $var->id }}" {{ $statehead_id == $var->id ? 'selected' : '' }} >
                        ...
                        <div class="col-lg-6">
                            <fieldset>
                                ...
                                <textarea class="form-control" cols="" rows="1" placeholder="Enter Address" name="address" id="address">{{ empty($user) ? '' : $user->agent_prfile->address }}</textarea>
                            </fieldset>
                        </div>
                            ...
                            <fieldset>
                                ...
                                <div id="brand_logoDiv">
                                    @if (!empty($user->agent_prfile->brand_logo_file))
                                        <a href="javascript:void(0)" data-img="{{ asset($user->agent_prfile->brand_logo_file) }}" onclick="viewImage(this)">View file</a>
                                    @endif
                                </div>
                            </fieldset>
                        </div>
                        ...
                        <div class="col-lg-6">
                            <fieldset>
                                <div class="form-group">
                                    <label for="drug_licence_file">Identity proof <small>(only jpg,jpeg,png or maximum file size 2MB)</small></label>
                                    <div class="custom-file">
                                ...
                                <div id="drug_licenceDiv">
                                    @if (!empty($user->agent_prfile->drug_licence_file))
                                        <a href="javascript:void(0)" data-img="{{ asset($user->agent_prfile->drug_licence_file) }}" onclick="viewImage(this)">View file</a>
                                    @endif
                                </div>
                            </fieldset>
                        </div>
        ...
        $(function(){
            ...
            $('#agent_form').submit(function (e) { 
                ...
                if ($('#agent_form').parsley().isValid()) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{ route('admin.manage-user.agent.create') }}",
            ...
            $('#agent_form').submit(function (e) { 
                ...
                if ($('#agent_form').parsley().isValid()) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{ route('admin.manage-user.agent.edit', ['id'=>$user->id]) }}",
                        data: new FormData(this),
                        cache: false,
                        processData: false,
                        contentType: false,
                        ...
                        success: function (response) {
                            if (response.type == 'success') {
                                ...
                                const brand_logo_file = files.brand_logo_file;
                                const drug_licence_file = files.drug_licence_file;
                                if (brand_logo_file != null) {
                                    let htmlString = `<a href="javascript:void(0)" data-img="${brand_logo_file}" onclick="viewImage(this)">View file</a>`;
                                    $('#brand_logoDiv').html(htmlString);
                                }
                                if (drug_licence_file != null) {
                                    let htmlString = `<a href="javascript:void(0)" data-img="${drug_licence_file}" onclick="viewImage(this)">View file</a>`;
                                    $('#drug_licenceDiv').html(htmlString);
                                }
