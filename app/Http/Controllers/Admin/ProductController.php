...
class ProductController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = \App\Models\Product::where('admin_verify', '1')->select($selectField)->withTrashed()->get();
            return DataTables::of($data)
                ...
                ->addColumn('actions', function ($data) {
                    ...
                    if ($data->trashed()) {
                        ...
                        $btn .= '<a target="_blank" href="' . url("admin/product/view") . '/' . $data->id . '" class="btn-ancher"><i class="fa fa-eye fa-lg"></i></a>';
                        ...
                        $btn = '<a href="' . route('admin.product.edit', ['id' => $data->id]) . '" class="btn-ancher"><i class="fa fa-pencil-square-o fa-lg"></i></a>';
    ...
    public function edit(Request $request, $id)
    {
        ...
        if ($request->ajax()) {
            ...
            if (blank($product)) {
                return ['type' => 'redirect', 'url' => route('admin.product.index'), 'message' => 'This product is already delected'];
            }
    ...
    {
        if ($type == 'pending') {
            $data['list'] = \App\Models\Product::where('admin_verify', '2')->withTrashed()->orderBy('id', 'DESC')->paginate(10);
            $data['list_title'] = 'Pending Products';
        } elseif ($type == 'rejected') {
            $data['list'] = \App\Models\Product::where('admin_verify', '0')->withTrashed()->orderBy('id', 'DESC')->paginate(10);
            $data['list_title'] = 'Rejected Products';
        } elseif ($type == 'all') {
            $data['list'] = \App\Models\Product::where('admin_verify', '1')->withTrashed()->orderBy('id', 'DESC')->paginate(10);
    ...
    {
        $data = \App\Models\Product::where('id', $id)->withTrashed()->first();

        if (!empty($data)) {

            if ($status == 1 and \App\Models\Product::where('id', $data->id)->withTrashed()->update(['admin_verify' => $status])) {
                $response = [
                    'status'    =>  1,
                    'message'   =>  'Product Approve Succeddfully.',
                ];
            } elseif ($status == 0 and \App\Models\Product::where('id', $data->id)->withTrashed()->update(['admin_verify' => $status])) {
                $response = [
                    'status'    =>  1,
                    'message'   =>  'Product Rejected Succeddfully.',
                ];
            } else {
                $response = [
                    'status'    =>  0,
    ...
    {
        $product = \App\Models\Product::where('id', $id)->withTrashed()->first();
        if (blank($product)) {
            return redirect()->route('admin.product.index');
        }
