...
class ManageDeliveryBoyController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $data = \App\Models\User::select($select)->join('deliveryboy_profiles','deliveryboy_profiles.deliveryboy_id', '=','users.id')->where('users.user_type', $user_type)->where('deliveryboy_profiles.agent_id', auth()->user()->id)->get();
            ...
            return DataTables::of($data)
                ...
                ->addColumn('actions', function($data) {
                    ...
                    if ($data->trashed()) {
                        ...
                        $btn = '<a href="'.route('agent.manage-user.deliveryboy.edit', ['id'=>$data->id]).'" class="btn-ancher"><i class="fa fa-pencil-square-o fa-lg"></i></a>';
        ...
        return view('agent.manage-user.deliveryboy.index', $data);
    }
    public function create(Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($validator->fails()) {
                ...
                if ($user->save()) {
                    ...
                    $deliveryboyProfile->agent_id = auth()->user()->id;
                    $deliveryboyProfile->statehead_id = (empty(auth()->user()->agent_prfile->statehead_id)) ? NULL : auth()->user()->agent_prfile->statehead_id;
        ...
        $data['allstates'] = \App\Models\State::all();
        return view('agent.manage-user.deliveryboy.deliveryboy', $data);
    }
    public function edit(Request $request, $id)
    {
        ...
        if ($request->ajax()) {
            ...
            if (blank($user)) {
                return ['type'=>'redirect', 'url'=> route('agent.manage-user.deliveryboy.index'),'message'=>'This delivery boy is already delected' ];
            }
            ...
            if ($validator->fails()) {
                ...
                if ($user->save()) {
                    ...
                    $user->deliveryboy_profile->statehead_id = (empty(auth()->user()->agent_prfile->statehead_id)) ? NULL : auth()->user()->agent_prfile->statehead_id;
        ...
        if (blank($user)) {
            return redirect()->route('agent.manage-user.deliveryboy.index');
        }
